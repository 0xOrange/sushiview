...
var Pair = /*#__PURE__*/function () {
  function Pair(FACTORY_ADDRESS, INIT_CODE_HASH, tokenAmountA, tokenAmountB) {
    var tokenAmounts = tokenAmountA.token.sortsBefore(tokenAmountB.token) // does safety checks
    ? [tokenAmountA, tokenAmountB] : [tokenAmountB, tokenAmountA];
    this.liquidityToken = new Token(tokenAmounts[0].token.chainId, Pair.getAddress(FACTORY_ADDRESS, INIT_CODE_HASH, tokenAmounts[0].token, tokenAmounts[1].token), 18, 'UNI-V2', 'Uniswap V2');
    this.tokenAmounts = tokenAmounts;
  }

  Pair.getAddress = function getAddress(FACTORY_ADDRESS, INIT_CODE_HASH, tokenA, tokenB) {
    var _pairCache$tokens$0$a;

    var tokens = tokenA.sortsBefore(tokenB) ? [tokenA, tokenB] : [tokenB, tokenA]; // does safety checks

    var pairCache = FACTORY_CACHE === null || FACTORY_CACHE === void 0 ? void 0 : FACTORY_CACHE[FACTORY_ADDRESS];

    if (pairCache === undefined || (pairCache === null || pairCache === void 0 ? void 0 : (_pairCache$tokens$0$a = pairCache[tokens[0].address]) === null || _pairCache$tokens$0$a === void 0 ? void 0 : _pairCache$tokens$0$a[tokens[1].address]) === undefined) {
      var _extends2, _extends3;

      FACTORY_CACHE[FACTORY_ADDRESS] = _extends({}, pairCache, (_extends3 = {}, _extends3[tokens[0].address] = _extends({}, pairCache === null || pairCache === void 0 ? void 0 : pairCache[tokens[0].address], (_extends2 = {}, _extends2[tokens[1].address] = address.getCreate2Address(FACTORY_ADDRESS, solidity.keccak256(['bytes'], [solidity.pack(['address', 'address'], [tokens[0].address, tokens[1].address])]), INIT_CODE_HASH), _extends2)), _extends3));
    }

    return FACTORY_CACHE[FACTORY_ADDRESS][tokens[0].address][tokens[1].address];
  }
  /**
  ...
  _proto.getOutputAmount = function getOutputAmount(FACTORY_ADDRESS, INIT_CODE_HASH, inputAmount) {
    ...
    return [outputAmount, new Pair(FACTORY_ADDRESS, INIT_CODE_HASH, inputReserve.add(inputAmount), outputReserve.subtract(outputAmount))];
  };

  _proto.getInputAmount = function getInputAmount(FACTORY_ADDRESS, INIT_CODE_HASH, outputAmount) {
    ...
    return [inputAmount, new Pair(FACTORY_ADDRESS, INIT_CODE_HASH, inputReserve.add(inputAmount), outputReserve.subtract(outputAmount))];
  };
...
var Trade = /*#__PURE__*/function () {
  function Trade(FACTORY_ADDRESS, INIT_CODE_HASH, route, amount, tradeType) {
    ...
    if (tradeType === exports.TradeType.EXACT_INPUT) {
      ...
      for (var i = 0; i < route.path.length - 1; i++) {
        ...
        var _pair$getOutputAmount = pair.getOutputAmount(FACTORY_ADDRESS, INIT_CODE_HASH, amounts[i]),
      ...
      for (var _i = route.path.length - 1; _i > 0; _i--) {
        ...
        var _pair$getInputAmount = _pair.getInputAmount(FACTORY_ADDRESS, INIT_CODE_HASH, amounts[_i]),
  ...
  Trade.exactIn = function exactIn(FACTORY_ADDRESS, INIT_CODE_HASH, route, amountIn) {
    return new Trade(FACTORY_ADDRESS, INIT_CODE_HASH, route, amountIn, exports.TradeType.EXACT_INPUT);
  }
  /**
  ...
  Trade.exactOut = function exactOut(FACTORY_ADDRESS, INIT_CODE_HASH, route, amountOut) {
    return new Trade(FACTORY_ADDRESS, INIT_CODE_HASH, route, amountOut, exports.TradeType.EXACT_OUTPUT);
  }
  /**
  ...
  Trade.bestTradeExactIn = function bestTradeExactIn(FACTORY_ADDRESS, INIT_CODE_HASH, pairs, currencyAmountIn, currencyOut, _temp, // used in recursion.
  currentPairs, originalAmountIn, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      try {
        ...
        var _pair$getOutputAmount2 = pair.getOutputAmount(FACTORY_ADDRESS, INIT_CODE_HASH, amountIn);
      ...
      if (amountOut.token.equals(tokenOut)) {
        sortedInsert(bestTrades, new Trade(FACTORY_ADDRESS, INIT_CODE_HASH, new Route([].concat(currentPairs, [pair]), originalAmountIn.currency, currencyOut), originalAmountIn, exports.TradeType.EXACT_INPUT), maxNumResults, tradeComparator);
      } else if (maxHops > 1 && pairs.length > 1) {
        var pairsExcludingThisPair = pairs.slice(0, i).concat(pairs.slice(i + 1, pairs.length)); // otherwise, consider all the other paths that lead from this token as long as we have not exceeded maxHops

        Trade.bestTradeExactIn(FACTORY_ADDRESS, INIT_CODE_HASH, pairsExcludingThisPair, amountOut, currencyOut, {
  ...
  currentPairs, originalAmountOut, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      try {
        ...
        var _pair$getInputAmount2 = pair.getInputAmount(FACTORY_ADDRESS, INIT_CODE_HASH, amountOut);
      ...
      if (amountIn.token.equals(tokenIn)) {
        sortedInsert(bestTrades, new Trade(FACTORY_ADDRESS, INIT_CODE_HASH, new Route([pair].concat(currentPairs), currencyIn, originalAmountOut.currency), originalAmountOut, exports.TradeType.EXACT_OUTPUT), maxNumResults, tradeComparator);
      } else if (maxHops > 1 && pairs.length > 1) {
...
var Fetcher = /*#__PURE__*/function () {
  ...
  Fetcher.fetchPairData = function fetchPairData(FACTORY_ADDRESS, INIT_CODE_HASH, tokenA, tokenB, provider) {
    try {
      ...
      var address = Pair.getAddress(FACTORY_ADDRESS, INIT_CODE_HASH, tokenA, tokenB);
      return Promise.resolve(new contracts.Contract(address, IUniswapV2Pair.abi, provider).getReserves()).then(function (_ref) {
        ...
        return new Pair(FACTORY_ADDRESS, INIT_CODE_HASH, new TokenAmount(tokenA, balances[0]), new TokenAmount(tokenB, balances[1]));
      });
    } catch (e) {
      return Promise.reject(e);
    }
